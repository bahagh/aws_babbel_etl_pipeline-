...
def lambda_handler(event, context):
    try:
        for record in event['Records']:
            ...
            # Checking if event has been processed before (if the uuid is existing in the dynamodb table that i'm using as cache to store the uuid of the events which have been already processed and stored in s3), if yes the rest of the code in the loop will be skipped, won't store the event in s3 again and will move to process the next event
            ...
            if 'Item' in response:
                ...
                return {
                    'statusCode': 200,
                    'body': json.dumps('Event Skipped!')
                }
            else:
                logger.info(f'Event {event_uuid} is being processed .')
                ...
                # Constructing S3 key with meaningful format (used meaningful format to organize the data inside the s3 bucket by event type and to be able to distinguish data of same type with its creation time and its uid)
                ...
                return {
                    'statusCode': 200,
                    'body': json.dumps('Event processed and saved to S3 successfully!')
                }
